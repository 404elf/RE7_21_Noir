...
恶意房主无法再让客户端崩溃或耗尽日志空间：拒绝非法胜者、回合编号、事件参与者与异常卡牌计数，限制事件类型、字段与体积，并为去重集合、界面记录、单条记录和日志文件设定上限，且不删除已有日志。完整报告见 [SECURITY.md](SECURITY.md)。

准备倒计时改为只属于开局第一回合，「再来一局」与首次开局共用同一套初始化（起手王牌沿用预设数量与每局补充，不再写死），卡牌名称「归还+ADD2」更正为「加注2+」。极难 AI 补强牌桌满位、封锁与多张效果叠加时的决策。详见 [v1.5.2 修复验证](docs/RELEASE-v1.5.2.md)。

## v1.5.1：人机独立端口与加注规则

人机房间从共用固定端口改为独立本机端口并只监听回环地址，启动重试窗口延长至 15 秒并检查子进程是否意外退出；加注 1 / 2 只增加赌注，不再自带额外王牌，收割的独立奖励不变。107 项自动测试通过。详见 [v1.5.1 修复验证](docs/RELEASE-v1.5.1.md)。

## v1.5.0：准备阶段与分类下载包

卡牌说明点击优先、日志改为你 / 对手；修复数字牌后盲抽顺序；新增独立准备时间与计时预设；随机锁定、聊天 AI 配置说明及分类下载包。包含 v1.4.5 的确定领先等待策略。104 项自动测试通过。详见 [v1.5.0 验证记录](docs/RELEASE-v1.5.0.md)。

## v1.4.5：确定领先时先等对手

困难和极难 AI 在所有可能底牌下都领先时，先保留数字牌，让对手行动后再判断；不会把概率较高或可能平局当成必赢。对手补牌或用牌后重新计算，仍可加注和反制。

95 项测试通过；96 场同种子换边对照为 52 胜 44 负，仅显示小幅收益。完整分组和限制见 [AI 验证说明](docs/AI-v1.4.5.md)。

## v1.4.4：文案与极难试探策略

界面文案更简洁，卡牌说明统一使用赌注和抽牌用语；英文显示名采用已核实的原版名称。极难 AI 在关键局面可用数字牌试探底牌，成功抽牌导致爆牌时立即归还；王牌不足、缺少归还或信息价值不足时不主动使用此组合。原有留牌权重保留，试验性提高权重未采用。

详见 [文案与 AI 说明](docs/AI-v1.4.4.md)。

## v1.4.3：避免 AI 空放已知无效的数字牌

困难和极难 AI 不再为了抽取自己的底牌或已亮出的数字而空放纯数字王牌；仍可试探未知数字，并保留加强数字牌、收割和解除强迫消耗的有效组合。需要腾位置时仍可弃牌。每次用牌后重新判断可用数字，兼容归还等牌面变化。

## v1.4.2：修复 AI 提前停牌和盲目加注

困难和极难共用修复：低点数安全抽牌优先；Return 后继续评估补牌；落后时比较最多五次后续抽牌机会，避免只因一张牌仍不能反超就认输。指定数字抽牌失败、完美抽牌等明确线索现在也用于困难模式。

加注要求较稳定的优势，不能把之后随机抽到好牌当作提前加注的理由。避免为随后盲抽而提前用目标牌、Return 或在安全低点数阶段消耗指定数字牌。直接凑到目标点数、解除爆牌及确定的加注斩杀仍可用。

新增截图局面及出牌顺序回归测试；原卡牌规则和联机协议未改。详细验证见 [v1.4.2 AI 修复说明](docs/AI-v1.4.2.md)。

## v1.4.1：处刑者与对局追溯

- 困难的保守、赌徒、摇摆型均使用新版策略：按实际卡牌规则评估短连招，修正破坏牌类型匹配，考虑生命、加注、反制与留牌。赌徒保持额外冒险倾向。
- 新增「极难 · 处刑者」，不受风格选择限制。基于公开动作更新底牌概率，使用完美抽牌、指定抽牌的成功/失败以及加注/停牌等信号；搜索最多三张连续王牌，评估留牌、反制、斩杀、欲望下弃牌和目标切换。随机假设对手王牌，绝不读取实际底牌、对方王牌身份或真实牌堆顺序。
- 极难获胜/落败有专属结算：YOU SURVIVED / YOU DIED，侧栏展示挑战结果，保持双方牌面可见。
- 再来一局从第 1 回合开始，清除上一场行动冷却与开局资料。
- 战斗日志记录双方起手：本人首牌数字、对方未亮首牌 ?；王牌导致的明牌变化、目标变化和结算完整亮牌也会记录。本地日志按当前座位保存，不把对方起手暗牌提前写入文件。

策略是有限深度的概率规划，并非穷举解算器。行为推测可能被诈唬欺骗，未知王牌按配置抽样，特殊自定义规则会影响表现。目标是增加挑战；与熟手五五开的水平仍需真人对局验证，自动对战胜率不能代替这一验证。测试和校准见 [AI 验证说明](docs/AI-v1.4.1.md)。

## v1.4.0：联机选项与网络安全更新

大厅「选择联机方式」提供局域网搜索、房间码、地址直连、自建服务器收藏。房间码支持密码、规则预览、准备和 30 秒断线重连。

**房间码需要部署服务器，本项目目前没有预配置公共服务。** 下载包附服务端、配置和启动脚本，详见 [联机部署说明](NETWORKING.md)。本轮没有自动 P2P 穿透；房间码采用服务器方案。

**旧版网络协议存在不安全的 pickle 解析，v1.4.0 已替换为受限 JSON，不再兼容旧版联机。** 双方和服务器都应更新。卡牌规则类保持不变，改动的是网络收发与会话层。详见 [安全检查报告](SECURITY.md)。以下旧版本介绍中的「原服务端兼容」仅指历史版本，不适用于新版网络协议。
